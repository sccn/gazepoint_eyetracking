% EEGPLUGIN_GAZEPOINT_EYETRACKING() - EEGLAB plugin for importing GAZEPOINT eye-tracking data files.
%
% Usage:
...
function vers = eegplugin_gazepoint_eyetracking(fig, trystrs, catchstrs)
    ...
    % add folder to path
    % ------------------
    if ~exist('pop_read_gazepoint')
        p = fileparts(which('pop_read_gazepoint.m'));
        addpath(p );
    end;
    ...
    % menu callbacks
    % --------------
    comcnt = [ trystrs.no_check '[EEG, LASTCOM] = pop_read_gazepoint;' catchstrs.new_and_hist ];
    
    % create menus
    % ------------
    uimenu( menu, 'label', 'From GAZEPOINT eye-tracking file', 'callback', comcnt, 'separator', 'on' );
